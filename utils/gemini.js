...
// const model = genAi.getGenerativeModel(
//   {
//     // Good choices: "gemini-1.5-pro-002" (higher quality) or "gemini-2.5-flash" (faster/cheaper)
...
if (!process.env.GEMINI_API_KEY) {
  throw new Error("GEMINI_API_KEY is missing. Put it in .env");
}
  ...
  properties: {
    milestones: {
      ...
      items: {
        type: SchemaType.OBJECT,
        properties: {
          year: { type: SchemaType.STRING, description: "Target year", nullable: false },
          milestone: { type: SchemaType.STRING, description: "Milestone description", nullable: false },
          reduction: { type: SchemaType.NUMBER, description: "Reduction % at milestone", nullable: false },
          start: { type: SchemaType.NUMBER, description: "Start year", nullable: false },
          end: { type: SchemaType.NUMBER, description: "End year", nullable: false },
        },
        required: ["year", "milestone", "reduction", "start", "end"],
      },
    },
...
// --- SDK + model (stable v1 + current model). No extra env vars needed. ---
const genAi = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
const model = genAi.getGenerativeModel(
  {
    // Leave as-is, or swap to "gemini-2.5-pro" if you want higher quality.
  ...
  { apiVersion: "v1beta" } // important: use stable v1 to avoid v1beta 404s
);
...
exports.analyseRoadmap = async (
  ...
  try {
    const sampleOutput = {
      milestones: [
        { year: "2025", milestone: "Adopt 50% renewable energy", reduction: 20, start: 2024, end: 2025 },
        { year: "2027", milestone: "Switch 60% of fleet to EVs", reduction: 35, start: 2025, end: 2027 },
        { year: "2030", milestone: "Engage suppliers in carbon-neutral practices", reduction: 50, start: 2027, end: 2030 },
        { year: "2035", milestone: "Improve energy efficiency by 30%", reduction: 75, start: 2030, end: 2035 },
        { year: "2040", milestone: "Achieve Net Zero", reduction: 100, start: 2035, end: 2040 },
      ],
    };
    ...
    const prompt = `
      Given the following decarbonization roadmap milestones:
      ${JSON.stringify(sampleOutput, null, 2)}
      Generate a similar roadmap for the ${industry} industry to reach Net Zero by ${targetYear}
      starting from ${totalEmissions} tons of CO2 emissions.

      Additional Factors:
      - Emission Reduction Goals: ${annualReduction}% annually.
      - Current Energy Mix: ${energyMix} fossil vs renewables.
      - Technology Adoption: ${technologyAdoption}.
      - Operational Changes: ${operationalChanges}.
      - Budget Constraints: ${budgetConstraints} USD.
    ...
    console.error("[Gemini analyseRoadmap error]:", error?.message || error);
