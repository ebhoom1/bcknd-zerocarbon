...
exports.addMobileCombustion = async (req, res) => {
  try {
    console.log("mobile combustion emission facor:",req.body);
    const newRecord = new MobileCombustion(req.body);
    const savedRecord = await newRecord.save();
    res.status(201).json({
      message: "mobile combustion data added successfully",
      savedRecord,
    });
  } catch (error) {
...
exports.deleteMobileCombustion = async (req, res) => {
  try {
    const deletedRecord = await MobileCombustion.findByIdAndDelete(
      req.params.id
    );
    if (!deletedRecord)
      return res.status(404).json({ message: "Record not found" });
